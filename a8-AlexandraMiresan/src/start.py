...
def main():
    ...
    match repo_type:
        case "inmemory":
            ...
            populate_rental(rental_repo, client_repo.get_all_clients(), movie_repo.get_all_movies())
            ui = UI(movie_repo, client_repo, rental_repo)
        case "textfiles":
            ui = UI(TextFileMovieRepository(movie_repo_file), TextFileClientRepository(client_repo_file), TextFileRentalRepository(rental_repo_file))
        case "binaryfiles":
